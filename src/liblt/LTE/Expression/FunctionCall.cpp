...
namespace
{
  ...
  ArgData(Expression const &expression) : expression(expression),
...
ExpressionFunctionCall(
    ...
    Vector<Expression> const &arguments) : function(function)
{
...
namespace LTE
{
      ...
      CompileEnvironment &env)
  {
    ...
    {
      if (env.detail)
        Log_Message(Stringize() | "functioncall -- no candidate functions '" | name | "'");
      return nullptr;
    }
    ...
    {
      if (env.detail)
        Log_Message(Stringize() | "functioncall -- no overloads of '" | name | "' were compatible with arguments");
      return nullptr;
    }
