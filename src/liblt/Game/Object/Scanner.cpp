...
typedef ObjectWrapper<Component_Pluggable<Component_Supertyped<ObjectWrapperTail<ObjectType_Scanner>>>>
    ScannerBaseT;
                 ...
                 float, interpolant)
{
  if (target.bands.size() != source.bands.size())
    error("Cannot mix scanner outputs of different resolutions") for (size_t i = 0; i < target.bands.size(); ++i)
        Mutable(target)
            .bands[i] = Mix(
